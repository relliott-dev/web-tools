...
Date: 2024-03-20
Identifies and reports broken image links on a webpage
...
# Finds images with broken links
...
def output_results(url, broken_images, output_file, format='txt', is_last=False):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if format == 'csv':
        with open(output_file, "a", newline='', encoding="utf-8") as file:
            csv_writer = csv.writer(file)
            csv_writer.writerow([timestamp, url, len(broken_images)] + broken_images)
        print(f"Results saved to {output_file}")
    elif format == 'json':
        result_data = {'timestamp': timestamp, 'url': url, 'total_broken_images': len(broken_images), 'broken_images': broken_images}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Broken Image Finder Script")
    ...
    with open("broken_image_finder.txt", "w", encoding="utf-8") as file:
        pass
    if export_csv:
        with open("broken_image_finder.csv", "w", newline='', encoding="utf-8") as file:
            csv_writer = csv.writer(file)
            csv_writer.writerow(['Timestamp', 'Website', 'Total Broken Images'] + ['Broken Image ' + str(i+1) for i in range(max(len(find_broken_images(format_url(url))) for url in urls))])
    if export_json:
        with open("broken_image_finder.json", "w", encoding="utf-8") as file:
            file.write('[')
            file.write("\n")
    ...
    for i, url in enumerate(urls):
        ...
        broken_images = find_broken_images(formatted_url)
        is_last_url = i == len(urls) - 1
        output_results(formatted_url, broken_images, "broken_image_finder.txt", is_last=is_last_url)
        if export_csv:
            output_results(formatted_url, broken_images, "broken_image_finder.csv", format='csv', is_last=is_last_url)
        if export_json:
            output_results(formatted_url, broken_images, "broken_image_finder.json", format='json', is_last=is_last_url)
    
    if export_json:
        with open("broken_image_finder.json", "a", encoding="utf-8") as file:
            file.write(']')
